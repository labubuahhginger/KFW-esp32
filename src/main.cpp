...
enum State { 
  ...
  FILES_MENU, LITTLEFS_LIST, RFID_MENU, RFID_EMULATION, FORMAT_CARD, RFID_DUMP_PROCESS, RFID_RECV, FILE_OPTIONS, FILE_CONTENT, WEBUI_ACTIVE, TV_B_GONE_MODE, SYSTEM_INFO, PWN_MODE, SNIFFERS_MENU, PROBE_SNIFFER, BEACON_SNIFFER, CH_GRAPH, IR_MENU, IR_RECV, IR_SEND_LAST, SETTINGS_SCREEN, WIFI_INFO_SCREEN
};
...
void loop() {
  ...
  if (n || ok) {
    ...
    if (n) {
      ...
      if (currentState == BEACON_COUNT_SEL) {
        ...
        else if (currentState == WIFI_MENU) s = 7; 
    ...
    if (ok) {
      ...
      // 3. Если мы не в "бегающем" режиме, а в меню — идем в switch
        ...
        switch(currentState) {
          ...
          case WIFI_MENU:
            if(menuIndex == 0) { currentState = WEBUI_ACTIVE; WiFi.begin("NeNet5", "Password0888"); while (WiFi.status() != WL_CONNECTED) delay(500); server.begin(); } 
  ...
  switch(currentState) {
    ...
    case WIFI_MENU: {
      const char* m[] = {"Start AP", "Scan", "Attacks", "Sniffers", "Graph", "KFWGotchi", "Back"};
      renderMenu("Wi-Fi", m, nullptr, 7, menuIndex);
      } break;
