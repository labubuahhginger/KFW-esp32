...
void loop() {
  buttons.update();
  if (currentState == WEBUI_ACTIVE || currentState == EVIL_PORTAL) server.handleClient();
  ...
  if (n || ok) {
    ...
    if (ok) {
      ...
      else if (currentState == EVIL_PORTAL) { WiFi.softAPdisconnect(true); server.stop(); currentState = WIFI_MENU; }
